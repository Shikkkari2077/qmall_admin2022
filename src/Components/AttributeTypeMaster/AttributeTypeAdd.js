...
class AttributeTypeAdd extends React.Component {
  state = {
    status: true,
    description: ""
  };
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.attribute_type_id !== this.props.attribute_type_id) {
      this.setState({ attribute_type_id: this.props.attribute_type_id });
      this.getAttributeTypeDetails();
    }
    if (prevProps.language_id !== this.props.language_id) {
      if (this.props.attribute_type_id !== undefined) {
        // this.setState({ attribute_type_id: this.props.attribute_type_id });
        this.getAttributeTypeDetails();
      }
    }
  ...
  getAttributeTypeDetails = () => {
    ...
    data.append("AttributeTypeId", that.props.attribute_type_id);
    ...
    fetch(Constant.getAPI() + "/attribute/type/get", {
      ...
      if (json.status === true) {
        that.setState({
          attribute_tye_data: json.data,
          attribute_type_name: json.data[0].name_en,
          attribute_type_name_ar: json.data[0].name_ar
  ...
  onSaveData = () => {
    ...
    // }
    if (that.props.attribute_type_id !== undefined) {
  ...
  updateCategoryData = () => {
    ...
    var data = new URLSearchParams();
    this.setState({ isSaving: true });
    data.append("name_en", that.state.attribute_type_name);
    data.append("name_ar", that.state.attribute_type_name_ar);

    data.append("AttributeTypeId", that.props.attribute_type_id);
    data.append("LanguageId", that.props.language_id);
    fetch(Constant.getAPI() + "/attribute/type/update", {
      method: "post",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
        "Authorization": localStorage.getItem('q8_mall_auth')
      ...
      if (json.status === true) {
        Swal.fire("Updated !", "Attribute Type has been Updated", "success");
  ...
  addCategory = () => {
    ...
    // data.append("LanguageId", that.props.language_id);
    fetch(Constant.getAPI() + "/attribute/type/add", {
      method: "post",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
        "Authorization": localStorage.getItem('q8_mall_auth')
          ...
          <div className="row">
            <div className="col-md-6">
              <div className="form-group row">
                <label className="col-sm-3 col-form-label">Attribute Type Name (English)</label>
                <div className="col-sm-9">
                  <input
                    ...
                    value={this.state.attribute_type_name}
                  />
                </div>
              </div>
            </div>
            <div className="col-md-6">
              <div className="form-group row">
                <label className="col-sm-3 col-form-label">Attribute Type Name (Arabic)</label>
                <div className="col-sm-9">
                  <input
                    type="text"
                    className="form-control"
                    name="attribute_type_name_ar"
                    id="attribute_type_name_ar"
                    placeholder="Attribute Type Name"
                    onChange={this.handleChange}
                    value={this.state.attribute_type_name_ar}
                  />
                </div>
              </div>
            </div>
          ...
          */}
          <div className="card-footer">
            <div className="row float-right p-3">
              ...
              <Link to={"/attribute-types"} className="btn btn-outline-dark">
                Cancel
        </Link>
            </div>
          </div>
